...
// NEW: Command lookup table
std::map<std::string, std::string> command_lookup_table;
std::mutex lookup_table_mutex;
...
// =============================================================================
// NEW: Command Lookup Table Functions
// =============================================================================

// Helper: Normalize string (lowercase, trim whitespace)
...
// Helper: Remove filler words for fuzzy matching
...
// Helper: Calculate word match score
int calculate_match_score(const std::string& input, const std::string& key) {
    ...
    while (key_stream >> word) {
        key_words.push_back(word);
    }
    
    int matches = 0;
    for (const auto& iword : input_words) {
        for (const auto& kword : key_words) {
            if (iword == kword) {
                matches++;
                break;
            }
...
bool load_command_mappings(const std::string& filepath) {
    std::ifstream file(filepath);
    
    if (!file.is_open()) {
        std::cerr << "[COMMANDS] Failed to open " << filepath << std::endl;
        return false;
    }
    
    std::lock_guard<std::mutex> lock(lookup_table_mutex);
    command_lookup_table.clear();
    ...
    while (std::getline(file, line)) {
        ...
        if (eq_pos == std::string::npos) {
            std::cerr << "[COMMANDS] Line " << line_num << ": Invalid format (missing '=')" << std::endl;
            continue;
        }
        
        // Split into phrase and command
        std::string phrase = line.substr(0, eq_pos);
        std::string command = line.substr(eq_pos + 1);
        
        // Normalize phrase (case-insensitive key)
        phrase = normalize_string(phrase);
        
        // Trim command but keep its case
        size_t cmd_start = 0;
        while (cmd_start < command.length() && std::isspace(command[cmd_start])) {
            cmd_start++;
        }
        size_t cmd_end = command.length();
        while (cmd_end > cmd_start && std::isspace(command[cmd_end - 1])) {
            cmd_end--;
        }
        command = command.substr(cmd_start, cmd_end - cmd_start);
        
        if (phrase.empty() || command.empty()) {
            std::cerr << "[COMMANDS] Line " << line_num << ": Empty phrase or command" << std::endl;
            continue;
        }
        
        command_lookup_table[phrase] = command;
        loaded++;
    }
    
    file.close();
    
    std::cout << "[COMMANDS] Loaded " << loaded << " command mappings from " << filepath << std::endl;
    
    // Print loaded commands for verification
    if (loaded > 0) {
        std::cout << "[COMMANDS] Command mappings:" << std::endl;
        for (const auto& pair : command_lookup_table) {
...
// Fuzzy match command with input text
std::string fuzzy_match_command(const std::string& input_text) {
    std::lock_guard<std::mutex> lock(lookup_table_mutex);
    
    if (command_lookup_table.empty()) {
        return "";
    }
    
    // Normalize input
    std::string normalized_input = normalize_string(input_text);
    ...
    auto it = command_lookup_table.find(normalized_input);
    if (it != command_lookup_table.end()) {
        std::cout << "[MATCH] Exact match: \"" << normalized_input << "\" -> \"" << it->second << "\"" << std::endl;
        return it->second;
    }
    
    // Try fuzzy matching
    std::string input_no_filler = remove_filler_words(normalized_input);
    
    std::string best_match_command;
    int best_score = 0;
    std::string best_match_key;
    
    for (const auto& pair : command_lookup_table) {
        const std::string& key = pair.first;
        const std::string& command = pair.second;
        
        // Check if input contains key or key contains input
        if (normalized_input.find(key) != std::string::npos || 
            key.find(normalized_input) != std::string::npos) {
            std::cout << "[MATCH] Substring match: \"" << normalized_input << "\" contains/in \"" 
                      << key << "\" -> \"" << command << "\"" << std::endl;
            return command;
        }
        
        // Try with filler words removed
        std::string key_no_filler = remove_filler_words(key);
        if (input_no_filler == key_no_filler) {
            std::cout << "[MATCH] No-filler match: \"" << input_no_filler << "\" -> \"" 
                      << command << "\"" << std::endl;
            return command;
        }
        
        // Calculate word match score
        int score = calculate_match_score(input_no_filler, key_no_filler);
        if (score > best_score) {
            best_score = score;
    ...
    // If we have a reasonable match (at least 2 words match), use it
    if (best_score >= 2) {
        std::cout << "[MATCH] Fuzzy match (score=" << best_score << "): \"" << normalized_input 
                  << "\" ~= \"" << best_match_key << "\" -> \"" << best_match_command << "\"" << std::endl;
                            ...
                            const std::string& commands_file) 
{
    ...
    load_command_mappings(commands_file);
    ...
    while (keep_running) {
        ...
        // Check for commands file reload request
        if (commands_update_requested) {
            commands_update_requested = 0;
            load_command_mappings(commands_file);
        }
        
        // NEW: Check trigger timeout in Mode 1
        ...
        if (!audio_data.empty()) {
            ...
            if (final) {
                ...
                if (!final_text.empty()) {
                    if (current_mode == 1) {
                        // MODE 1: Grammar-based command mode with trigger + lookup
    ...
    if (!command_after_trigger.empty()) {
        ...
        std::string matched_command = fuzzy_match_command(command_after_trigger);
        
        if (!matched_command.empty()) {
            if (send_command_to_server(matched_command, "127.0.0.1", 6975)) {
                std::cout << "[COMMAND] ✓✓✓ ONE-STEP COMMAND SENT: \"" << matched_command << "\" ✓✓✓" << std::endl;
            } else {
                std::cerr << "[COMMAND] ✗✗✗ FAILED to send one-step command: \"" << matched_command << "\" ✗✗✗" << std::endl;
            }
        } else {
        ...
        std::cout << "[TRIGGER] *** \"" << TRIGGER_PHRASE 
                  ...
                  << TRIGGER_TIMEOUT_MS << "ms ***" << std::endl;
                        ...
                        // 2. Check if we're in triggered state
                            ...
                            // NEW: Lookup command in table
                            std::string matched_command = fuzzy_match_command(final_text);
                            
                            if (!matched_command.empty()) {
                                // Send matched command to TCP server on port 6975
                                if (send_command_to_server(matched_command, "127.0.0.1", 6975)) {
                                    std::cout << "[COMMAND] ✓✓✓ COMMAND SENT to 127.0.0.1:6975: \"" << matched_command << "\" ✓✓✓" << std::endl;
                                    // Optional: Reset trigger after successful command?
                                    // command_triggered = false; 
                                } else {
                                    std::cerr << "[COMMAND] ✗✗✗ FAILED to send command: \"" << matched_command << "\" ✗✗✗" << std::endl;
                                }
                            } else {
                                // No match found - log and ignore
                                std::cout << "[COMMAND] ✗ No match for: \"" << final_text << "\" (ignored)" << std::endl;
                            }
                        } 
                        // 3. Not triggered
                        else {
...
Config cfg = parse_args(argc, argv);
    ...
    std::ifstream commands_check(cfg.commands_file);
    if (!commands_check.is_open()) {
        std::ofstream commands_create(cfg.commands_file);
        commands_create << "# Command Lookup Table" << std::endl;
        commands_create << "# Format: spoken_phrase=command_line" << std::endl;
        commands_create << "# Example:" << std::endl;
        commands_create << "turn on front led=config_led on" << std::endl;
        commands_create << "turn off front led=config_led off" << std::endl;
        commands_create << "set brightness fifty=config_brightness 50" << std::endl;
        commands_create << "turn on the light=config_led on" << std::endl;
        commands_create << "turn off light=config_led off" << std::endl;
        commands_create.close();
        std::cout << "[COMMANDS] Created commands file: " << cfg.commands_file << " with example mappings" << std::endl;
    } else {
        commands_check.close();
    }
